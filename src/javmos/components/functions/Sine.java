...
public final class Sine extends Trigonometric {
    ...
    public Sine(JavmosGUI gui, String function) {
        ...
        a = (function.indexOf('s') == 0) ? 1
                : (function.substring(0, function.indexOf('s')).equals("-"))
                ? -1 : Double.valueOf(function.substring(0, function.indexOf('s')));
    ...
    public String getFirstDerivative() {
        /*
         * This function returns the first derivative of a sine
         ...
         */
        return "f'(x) = " + (a * k == 1 ? "" : (a * k == -1 ? "-" : a * k)) + "cos(" + (k == 1 ? "" : (k == -1 ? "-" : k)) + "x)";
    }

    public String getSecondDerivative() {
        /*
         * This function returns the second derivative of a sine
         * Again, ternary operators below check if prefixes are 1/-1 and acts accordingly
         */
        return "f''(x) = " + (a * k * -k == 1 ? "" : (a * k * -k == -1 ? "-" : a * k * -k)) + "sin(" + (k == 1 ? "" : (k == -1 ? "-" : k)) + "x)";
    }

    public double getValueAt(double x, FunctionType functionType) {
        double value = 0;
        if (functionType == FunctionType.ORIGINAL) {
            value = a * Math.sin(k * x);
        } else if (functionType == FunctionType.FIRST_DERIVATIVE) {
            value = a * k * Math.cos(k * x);
        } else if (functionType == FunctionType.SECOND_DERIVATIVE) {
            value = -a * k * k * Math.sin(k * x);
        } else {
            value = -a * k * k * k * Math.cos(k * x);
        }
        return value;
    }
